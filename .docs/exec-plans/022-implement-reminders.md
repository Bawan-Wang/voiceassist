...
- [x] Confirm [product-specs/reminders.md](../product-specs/reminders.md) is the
      finalized source of truth.
- [x] Snapshot the current baseline suite:
      `cd /home/jh-pi/.openclaw/workspace/voiceassist && .venv/bin/pytest -q`
- [x] Confirm there is no active overlapping reminder implementation branch or
      unmerged reminder module under `src/api/skills/`.
- [x] Re-read the current time-query implementation in
...
- [x] Add a dependency-light reminder module under `src/api/skills/`.
- [x] Parse reminder intent, task text, and time shape for:
  - absolute reminders,
  - relative minute/hour reminders,
  - timezone-aware clock reminders.
- [x] Reuse or extract the curated timezone alias table from the time-query
      implementation instead of copying it into a second independent table.
- [x] Represent the parser result as explicit local-code outcomes:
  - immediate create,
  - `need_time_detail`,
  - `confirm_candidate`,
  - rejection.
...
- [x] Add atomic JSON store helpers for:
  - `data/reminders.json`
  - `data/reminder_pending.json`
- [x] Keep durable reminders and short-lived pending state in separate files.
- [x] Store timestamps as timezone-aware ISO 8601 values.
- [x] Ensure absent files are treated as empty state, not hard failures.

### Step 3 — Extend shared routing policy with reminder precedence

- [x] Add a reminder route kind to `src/api/skills/policy.py`.
- [x] Lock the new route order to:
  1. local skill
  2. reminder
  3. time query
  4. tool-needed/search
  5. chat
- [x] Add guard coverage proving reminder phrases that contain time/timezone
      words do not fall into the time-query route.

### Step 4 — Wire the API execution path

- [x] Update `src/api/app.py` so `POST /zero-assistant` can:
  - create reminders,
  - emit clarification replies,
  - accept pending reminder follow-ups,
  - return truthful rejection replies.
- [x] Preserve the existing `AssistResponse` envelope.
- [x] Return `meta.source="local-skill"` for reminder create/confirm/cancel
      responses.

### Step 5 — Wire the voice bridge for pending follow-ups and due delivery

- [x] Update `src/bridge/voice_bridge.py` so an active pending reminder state
      is checked before normal stateless classification.
- [x] Keep wake-word handling, `_should_route_without_wake()`, and audio
      capture behavior unchanged.
- [x] Add a lightweight background due-reminder poller.
- [x] Deliver due reminders only when the bridge is idle.
- [x] Reuse `update_state(...)` and `speak(...)` for delivery UI/audio behavior.
- [x] Mark reminders delivered only after successful playback completion.

### Step 6 — Add reminder-focused tests

- [x] Add parser/normalizer unit coverage in a new reminder test file.
- [x] Add store tests covering atomic writes, TTL expiry, and durable reload.
- [x] Extend `tests/test_routing_policy.py` with reminder precedence cases.
- [x] Extend `tests/test_api.py` with reminder create/confirm/cancel/reject
      coverage.
- [x] Add voice-bridge reminder tests for:
...
- [x] Update [context.md](../context.md) with implementation notes and final
      verification results.
- [x] Update technical docs only if the final implementation materially changes
      the current voice-vs-HTTP entrypoint explanation.

### Step 8 — Verify in layers

- [x] Run the new reminder unit/store tests first.
- [x] Run the reminder routing/API/voice slice next.
- [x] Run the required repository suite after editing `src/api/app.py` and
...
- [x] One-turn absolute reminders create durable reminder records.
- [x] One-turn relative minute/hour reminders create durable reminder records.
- [x] Reminder phrases with timezone labels route to the reminder path, not the
      time-query path.
- [x] `need_time_detail` and `confirm_candidate` both work with a 60-second TTL.
- [x] Bare yes/no follow-ups are accepted only for `confirm_candidate`.
- [x] Missing-task, unknown-timezone, invalid-time, past-time, and store-write
      failures return truthful local-skill replies.
- [x] Pending reminder follow-ups bypass normal routing in the voice bridge.
- [x] Due reminders are delivered oldest-first only when the bridge is idle.
- [x] Overdue reminders survive restart and are delivered after startup.
